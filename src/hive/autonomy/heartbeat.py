...
class Heartbeat:
    ...
    async def _tick_inner(self, now: float) -> dict:
        memory_projection = self._hive.reconcile_local_memory_projections()
        recovered_memory = memory_projection["applied"]
        if recovered_memory:
            log.info("heartbeat: recovered %d local Obsidian projection(s)", recovered_memory)
