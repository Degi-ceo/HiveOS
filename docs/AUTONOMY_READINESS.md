...
| Scheduler delivery | Cron and commitments require the TaskBoard's SQLite database before they enqueue. Each uses one `BEGIN IMMEDIATE` transaction for queue insert plus cursor advance, and post-insert/cursor-update abort tests prove rollback before a retry creates exactly one task. Stable occurrence keys remain as compatibility protection for historical partial rows. Windows-compatible spawned-process tests prove that two independent schedulers produce one cron or commitment occurrence. Lock-timeout and long-running contention evidence remain. | Partial |
| Durable approval/run journal | Approval IDs are persisted on waiting tasks; approve → done, reject/TTL/kill → canceled, and an unconfirmed post-approval result → requires_review. Durable request timestamps enforce TTL even after process-memory loss. A restart between durable approval and live-gate handoff is quarantined, never invoked. A shared SQLite kill control terminalizes pending and approved-but-unstarted work, rejects new snapshots, and atomically blocks a fresh process from starting execution after the stop. Spawned-process decision races leave exactly one terminal decision. Full per-provider receipts and resume checkpoints remain pending; until a provider offers an audited idempotency/receipt contract, an unconfirmed Mnemosyne delivery or invalidation is quarantined for manual review rather than retried. | Partial |
| Recovery tests | Deterministic task, approval, memory-projection, self-mod recipe crash points, lost response, and restart tests pass. A post-binding Mnemosyne crash is quarantined without a second remote write, and external running projections without a lease are also quarantined fail-closed. Telegram startup now quarantines interrupted `processing`, `reply_pending`, and `sending` states before accepting a webhook. The full cross-provider receipt matrix remains. | Partial |
| Operations | Verified SQLite online backup, integrity check, explicit restore, a non-overwriting restore drill, append-only aggregate operation evidence, and a non-effectful `hive shadow` collector are implemented and regression-tested. `scripts/windows/shadow-soak.ps1` verifies the source before and after a read-only shadow observation; Windows supervision installation and a 24–72h evidence soak remain. | Partial |
...
3. Run `hive state drill BACKUP --output NEW.sqlite` against a verified backup, then run a 24–72 hour soak with `& .\scripts\windows\shadow-soak.ps1`, restart, and fault-injection evidence. Run `hive pilot-doctor` before and after the pilot to retain only aggregate review counts. Investigate adverse aggregates but do not replay or repair task rows during the soak. The script requires an existing source DB, writes evidence only to a separate DB, and verifies source integrity both before and after every observation. Keep its exit status and evidence DB as release evidence; any failed source verification, nonzero exit status, or unexplained `requires_review` increase is a NO-GO until investigated.
