...
3. Run a 24–72 hour soak with `& .\scripts\windows\shadow-soak.ps1`, restart, and fault-injection evidence; investigate adverse aggregates but do not replay or repair task rows during the soak. The script requires an existing source DB, writes evidence only to a separate DB, and verifies source integrity both before and after every observation. Keep its exit status and evidence DB as release evidence; any failed source verification, nonzero exit status, or unexplained `requires_review` increase is a NO-GO until investigated.
