...
| Scheduler delivery | Cron and commitments use one SQLite transaction for the queue write plus cursor advance. Baseline rollback coverage exists, but same-database enforcement, post-insert/cursor-update failure injection, and two-scheduler contention evidence remain before the atomicity claim can be promoted. Stable occurrence keys remain as compatibility protection for historical partial rows. | Partial |
