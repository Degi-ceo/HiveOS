...
Each store self-initializes its schema (WAL). `core/sqlite_ops.py` creates verified online-backup snapshots, requires an explicit confirmation for live restore, and offers a non-overwriting restore drill to a new destination; `core/pilot_doctor.py` is a separate read-only aggregate readiness inspection. `core/doctor.py` verifies the DB is
present/openable; it does **not** duplicate store DDL (avoids drift — fixed in #14). The memory outbox uses owner-fenced claims and version ordering. Completed Mnemosyne-backed conversation turns become one idempotent `session` ledger record before any projection. Only local Obsidian writes are replay-safe; a manual-note conflict or any unconfirmed Mnemosyne `remember`/`invalidate` outcome is durably marked `requires_review`, because Mnemosyne exposes no idempotency receipt contract. Projection review reasons are bounded classifications, never provider exception text. `autonomy/shadow.py` is deliberately outside the runtime: it opens a source DB read-only, records only bounded task-state/kind/source and lease-health aggregates (never payloads or error text), and writes only to a separately selected evidence DB; explicit operator operations also create a separate append-only aggregate evidence record. It never constructs `HiveOS` or imports tool execution paths. `HiveOS.build()` quarantines interrupted self-mod recipes even when autonomy is disabled; the public restart helper additionally drains only deterministic local Obsidian projections and never replays work. Gateway startup separately fences non-terminal Telegram inbox states before accepting a webhook.
