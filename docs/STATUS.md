...
The code contains an SQLite task board, cron, commitments, a heartbeat, and a memory ledger, but the audited system does not yet provide every delivery and operational guarantee required for safe autonomous execution. Expired task leases and uncertain approved executions are now fail-closed rather than replayed, and scheduler delivery is transactional. Scheduler execution rejects a board on another SQLite database, post-insert/cursor-update abort tests prove no partial task is committed, and Windows `spawn` tests prove two independent schedulers create only one cron or commitment occurrence. Durable approval timestamps enforce TTL after process-memory loss; an approval interrupted between durable approval and live-gate handoff becomes `requires_review`, never executable work. The shared emergency stop terminalizes pending and approved-but-unstarted snapshots and prevents a fresh process from starting protected execution; a spawned decision race leaves exactly one durable decision. Failure-driven self-modification recipes are journaled and quarantined during normal startup even when the heartbeat remains disabled. A separate `hive shadow` command can now collect read-only task-inventory evidence without constructing the runtime or invoking tools. Verified backups now have a non-overwriting restore drill and a separate append-only aggregate operation-evidence log. Gateway startup fences incomplete Telegram inbox states, and authenticated `/health/telegram-inbox` plus `hive pilot-doctor` expose only aggregate review evidence. Remaining gates include provider receipts, Windows supervision installation, long-running lock-resilience evidence, and a 24–72 hour operational shadow soak.

The outbound boundary now also records a durable, content-free intent/receipt fingerprint for protected external messages. Interrupted or receipt-less sends move to owner review and are never retried automatically; a prior false-success path for missing/failed Telegram delivery is closed. Slack, Discord, and email inbound auto-replies remain disabled by default pending an equivalent durable inbox/outbox contract.

Experimental secondary-channel turns now share a durable, content-free state fence. Their aggregate health is included in `hive pilot-doctor`; a restart, failed send, or receipt-less response requires owner review and never resumes the turn. Discord interaction parsing and trusted email-envelope provenance remain pilot gates.

Memory operations hardening: the canonical outbox now quarantines every in-flight external delivery during startup/restart without replaying it; a live claim is revalidated immediately before each external call or managed projection write, and a crash after a Mnemosyne binding is quarantined without issuing a second remote write. Authenticated API and Telegram show only aggregate projection/review state, and edits to prior managed Vault history block later projections for owner review. This improves evidence and recovery, but does not relax any autonomy gate.

Capability firewall hardening: all file writes, including append and new-file creation, now require approval; sensitive reads deny dotenv, key/credential formats, and known secret-store directories before any tool or prefixed MCP reader runs. Protected Hive paths are anchored to the repository rather than the caller's current directory. At runtime, `execute_approved` requires a matching durable approved snapshot and execution marker, while learned-skill constituents always re-enter the executor instead of calling a tool directly. Inbound MCP calls already use that same boundary. Trusted stdio MCP configuration remains an owner-managed privileged integration because a configured command starts before a tool call exists.

Outbound MCP trust hardening: `HIVE_MCP_SERVER_IDS` now selects only named entries from the fixed `Config/mcp-trust.json` owner manifest; legacy command-line specs and `MNEMOSYNE_MCP_URL` do not start a client. Stdio entries require an absolute existing executable and receive only an explicit minimal environment. SSE entries require HTTPS and a manifest-pinned hostname. Remote descriptors are allowlisted and namespaced, successful clients are retained for orderly shutdown, and authenticated `/mcp/trust` returns aggregate-only evidence.
...
- do not run `hive heartbeat` against `data/hive.sqlite` or enable the deploy orchestrator service;
...
The detailed acceptance criteria, rollout sequence, and rollback procedure are in [`AUTONOMY_READINESS.md`](AUTONOMY_READINESS.md). The staged Telegram-pilot and human-reviewed self-development plan is in [`AUTONOMY_ROADMAP.md`](AUTONOMY_ROADMAP.md). This overrides older optimistic statements in historical sprint/build documents; those documents remain historical records, not operational instructions.
### Telegram Command Surface v1
...
See `docs/TELEGRAM_COMMANDS.md`.
### Offline Telegram memory-pilot evaluation\n\n`evals/datasets/telegram_memory_pilot.jsonl` is a six-case, synthetic offline gate for memory corrections, freshness, provenance, uncertain external delivery, safety gates, and Polish owner communication. It runs with the deterministic mock target and contains no real Telegram data or credentials. It is readiness evidence only; it neither contacts Telegram nor enables autonomy or self-modification.\n\n### Safe-learning evidence gate\n\n`telegram_safe_learning_v1` adds a versioned, deterministic offline contract for corrected recall, correction precedence, dangerous-request refusal, safe task planning, and Polish quality. Its runner persists only aggregate evidence (identity, digest, timestamps, outcome totals, and offline-only flag); no prompts, outputs, chats, or secrets are written. Missing, failed, non-offline, or stale evidence blocks the opt-in learning-loop diagnosis path. Authenticated API routes can run or inspect this local contract, while Telegram `/evals` is read-only. Passing it does not create a change, send a message, merge, deploy, or enable autonomy.\n\n### 2026-08-31 autonomy safety progress

`TaskBoard` now persists `worker_id` and `lease_until`. Heartbeat claims work with a unique worker identity and terminal transitions are owner-checked. Every autonomy path is also gated by an IANA-local execution window; an empty, malformed, or equal-ended window fails closed before a tick can schedule, claim, plan, or execute work. Both the single-tick and persistent-loop paths return or stop before preflight and task activity, and focused regression tests cover empty, malformed, and equal-ended windows. Every enabled, preflight-passing tick performs idempotent crash recovery before scheduling and requeues only rows with an explicit expired owner lease that were durably marked replay-safe at enqueue; all other expired rows are quarantined for review; active and legacy unleased RUNNING rows are intentionally preserved for operator review. Failure-driven self-modification recipes are atomically journaled with their failure cursor; interrupted recipes are quarantined for operator review and never replayed. Cron and commitment task insertion plus cursor advance now share a single SQLite transaction with rollback on failure. Task outcomes now retain distinct `failed`, `canceled`, and `waiting_approval` states, and only failures can be manually retried. Completed-task cleanup uses an inclusive age cutoff, including the explicit zero-age operator cleanup boundary. Cron and commitments now use stable occurrence idempotency keys, so a crash after task persistence and before cursor advance retries one durable task instead of creating a duplicate. Approval request snapshots, terminal decisions, and approved-tool execution markers are durable and fail closed. The marker is committed before a protected tool call; a confirmed result settles its linked task as `done`, while an interrupted or unconfirmed execution is quarantined as `requires_review` and is never replayed automatically. The canonical memory outbox also uses owner-fenced leases: startup, the public restart helper, and every heartbeat tick reconcile only deterministic local Obsidian `Hive-Shadow` projections, even while autonomy is disabled. Potentially external Mnemosyne deliveries are never auto-replayed after an expired lease, an exception, an empty receipt, or a rejected invalidation; each such operation is durable `requires_review` with an operator-visible error. The legacy direct-memory fallback is disabled whenever the canonical ledger is configured, so a partial canonical write cannot be duplicated outside the ledger. Completed Mnemosyne-backed conversation turns likewise become one idempotent `session` ledger record before projection rather than two direct remote writes. Self-improvement now consumes only fresh, durably cursor-tracked failures from allowlisted production task sources; its first observation records a high-water mark, so historical and test-origin failures cannot trigger an autonomous diagnosis. Failed tasks get bounded exponential backoff only when they were explicitly declared replay-safe; default-deny tasks never auto-retry. `hive shadow` opens the selected source DB using SQLite read-only mode plus `query_only`, writes only bounded task-state/source/kind and lease-health aggregates to a separate evidence DB, never reads payloads or error text, and excludes planning, claims, tool execution, Telegram, memory projection, and self-modification.
### Canonical memory correction

Owner-only Telegram `/correct <stable-key> | <claim> | <reason>` now provides the deterministic human correction path. It appends a ledger version, derives the human actor and idempotency key from the authenticated Telegram update, retains history, and reconciles only local Obsidian projection work. It never invokes the model. Delayed idempotency retries return their original version, and omitted observation time is validated against creation time so a claim cannot be born already expired. Mnemosyne metadata now includes the correction reason. The legacy wipe endpoint refuses requests while the canonical ledger is active, preventing a misleading deletion claim.
...
It is the source of truth for Mnemosyne and Obsidian projections. Immutable claim versions carry provenance, confidence, observation/freshness timestamps, veracity, and a human-correction link; corrections append a successor with a reason and never delete the original. The model memory tool now writes through this ledger before any projection. With a configured ledger, both local and Mnemosyne recall plus model prompt/prefetch injection select only its current non-expired version. Static prompt context excludes session transcripts and admits only trusted durable claim classes; query recall is labelled as untrusted reference data. An old local row or a stale remote projection cannot override a human correction.
