...
M8 transparent execution control plane is implemented on
`codex/m8-execution-control-plane` from current `origin/main`. It adds a bounded,
content-safe RunLedger projection: snapshot, recursive child-run tree, and cursorable
replay of already-public operator envelopes. The terminal gains local and `--gateway`
views for run snapshot/tree/watch plus `hive status --live`; gateway execution routes
are ordinary-token, read-only endpoints. The projection never returns session or
...
M9 specialist workforce foundation is implemented on `codex/m9-specialist-workforce`.
The existing named specialists now have a closed, code-enforced profile registry, and every
...
output is withheld pending independent review, and cancelled attempts are terminal. Unknown
...
- **Autonomy (M3):** durable SQLite TaskBoard (survives restart) + cron (croniter optional)
  ...
  Explicit and proactive-batch cancellation records `cancelled`, never `failed`, so clearing
  suggestions cannot manufacture failure-rate self-improvement triggers.
