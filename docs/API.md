...
Authenticated standalone slash commands are dispatched locally from the central registry; they are never passed to `hive.ask()`. V1 offers `/start`, `/help`, `/commands`, `/new`, `/reset`, `/status`, `/sessions`, `/resume`, `/memory`, `/tasks`, and `/approvals`. The registry also reconciles Telegram's native menu at gateway startup. See [`TELEGRAM_COMMANDS.md`](TELEGRAM_COMMANDS.md) for the safety contract and rollout stages.
